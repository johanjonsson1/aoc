...
public class Day3 : Day
{
    ...
    public override void PartOne()
    {
        ...
        foreach (var line in input)
        {
            ...
            var compartments = line.Chunk(compartmentSize).ToList();
            var sharedItem = compartments[0].Intersect(compartments[1]).Single();
            sum += GetPriority(sharedItem);
        }

        Console.WriteLine(sum); // 7597
    }
    ...
    public override void PartTwo()
    {
        ...
        var sum = 0;
        foreach (var elfTrio in input.Chunk(3))
        {
            var sharedItem = elfTrio[0].Intersect(elfTrio[1]).Intersect(elfTrio[2]).Single();
            sum += GetPriority(sharedItem);
        }

        Console.WriteLine(sum); // 2607
    }
    ...
    private static int GetPriority(char c)
    {
        ...
        if (char.IsUpper(c))
        {
            return uc.IndexOf(c) + 27;
        }

        return lc.IndexOf(c) + 1;
    }

    private static string Input = @"zBBtHnnHtwwHplmlRlzPLCpp
