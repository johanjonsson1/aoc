...
Your puzzle answer was 4931744.
*/

public class Day7 : Day
{
    public override string Title => "--- Day 7: Amplification Circuit ---";
    ...
    public override void PartOne()
    {
        ...
        var thrusterInput = bag.ToList()
            .OrderByDescending(o => o.ThrusterSignal)
            .First();
    ...
    public override void PartTwo()
    {
        ...
        var amplifiersSeries = new List<AmplifierSeries>();
        var bag = new ConcurrentBag<ThrusterResult>();
        CreateAllSeriesPart2(amplifiersSeries);
        Parallel.ForEach(amplifiersSeries, s =>
        {
    ...
    private void CreateAllSeriesPart2(List<AmplifierSeries> amplifiersSeries)
    {
...
public class AmplifierSeries
{
    ...
    public bool BelowFour => _a <= 4 &&
                             _b <= 4 &&
                             _c <= 4 &&
                             _d <= 4 &&
                             _e <= 4;

    public bool BetweenFiveAndNine => _a >= 5 && _a <= 9 &&
                                      _b >= 5 && _b <= 9 &&
                                      _c >= 5 && _c <= 9 &&
                                      _d >= 5 && _d <= 9 &&
                                      _e >= 5 && _e <= 9;

    public bool IsDifferent => _b != _a &&
                               _c != _a && _c != _b &&
                               _d != _a && _d != _b && _d != _c &&
                               _e != _a && _e != _b && _e != _c && _e != _d;

    public AmplifierSeries(int input)
    {
        var asString = input.ToString().PadLeft(5, '0');
        _a = (int)char.GetNumericValue(asString[0]);
        _b = (int)char.GetNumericValue(asString[1]);
        _c = (int)char.GetNumericValue(asString[2]);
        _d = (int)char.GetNumericValue(asString[3]);
        _e = (int)char.GetNumericValue(asString[4]);
    }
