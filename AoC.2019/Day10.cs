...
namespace AoC2019
{
    ...
    This line of sight can be at any angle, not just lines aligned to the grid or diagonally. The best location is the asteroid that can detect the largest number of other asteroids.
    ...
    (The only asteroid it cannot detect is the one at 1,0; its view of this asteroid is blocked by the asteroid at 2,2.) All other asteroids are worse locations; they can detect 7 or fewer other asteroids. 
    ...
    If there were another asteroid at the location of a capital letter, the locations marked with the corresponding lowercase letter would be blocked and could not be detected:
    ...
    */
    ...
    {
        public override string Title => "--- Day 10: Monitoring Station ---";

        public override void PartOne()
        {
            base.PartOne();
            var grid = CreateAsteroids();
            SetDetectableAsteroids(grid);

            var ordered = grid.OrderByDescending(o => o.DetectableAsteroids).ToList();

            Console.WriteLine(ordered.First().DetectableAsteroids);
        }
        ...
        public override void PartTwo()
        {
            ...
            var station = grid.OrderByDescending(o => o.DetectableAsteroids).First();
            var currentAsteroids = grid.OrderBy(o => station.Coordinate.GetAngle90DegreesOffset(o.Coordinate)).ToList();
            ...
            while (currentAsteroids.Any(a => a.Vaporized == false))
            {
                ...
                {
                    var angle = station.Coordinate.GetAngle90DegreesOffset(otherAsteroid.Coordinate);
                    ...
                    var matchingAsteroids = currentAsteroids
                        .Where(x => x.Vaporized == false &&
                                    station.Coordinate.GetAngle90DegreesOffset(x.Coordinate) == angle)
                        .OrderBy(o => station.Coordinate.GetDistance(o.Coordinate)).ToList();
                    ...
                    if (counter == 199)
                    {
                        ...
                        Console.Write(" @ x {0}, y {1}", closest.Coordinate.X, closest.Coordinate.Y);
                    }

                    counter++;
                    closest.Vaporized = true;
                    station.OccupiedAngles.Add(angle);
                }
            }
        }

        private static void SetDetectableAsteroids(List<Asteroid> grid)
        {
            foreach (var asteroid in grid)
            {
                foreach (var otherAsteroid in grid)
                {
                    if (ReferenceEquals(asteroid, otherAsteroid))
                    {
                        continue;
                    }
        ...
        private static List<Asteroid> CreateAsteroids()
        {
            ...
            for (var y = 0; y < input.Count; y++)
            {
                for (var x = 0; x < input[0].Length; x++)
                {
                    ...
                    grid.Add(new Asteroid(newCoord.X, newCoord.Y));
                }
            }
    ...
    {
        public readonly List<double> OccupiedAngles = new List<double>();
        public int DetectableAsteroids;
