...
namespace AoC2019
{
    ...
    Every reaction turns some quantities of specific input chemicals into some quantity of an output chemical. Almost every chemical is produced by exactly one reaction; the only exception, ORE, is the raw material input to the entire process and is not produced by a reaction.
    ...
    7 A, 1 D => 1 E
    7 A, 1 E => 1 FUEL
    The first two reactions use only ORE as inputs; they indicate that you can produce as much of chemical A as you want (in increments of 10 units, each 10 costing 10 ORE) and as much of chemical B as you want (each costing 1 ORE). 
    To produce 1 FUEL, a total of 31 ORE is required: 1 ORE to produce 1 B, then 30 more ORE to produce the 7 + 7 + 7 + 7 = 28 A (with 2 extra A wasted) required in the reactions to convert the B into C, C into D, D into E, and finally E into FUEL. 
    ...
    Your puzzle answer was 1967319.
    ...
    Your puzzle answer was 1122036.
    */

    public class Day14 : Day
    {
        public override string Title => "--- Day 14: Space Stoichiometry ---";
        ...
        public override void PartOne()
        {
            ...
            OptimizeGotOreLookup(distinctChemicals);

            var resultForFuel = distinctChemicals.FirstOrDefault(f => f.Id == "FUEL")?.FindCheapestReactionByOre(1);

            Console.WriteLine(resultForFuel);
        }

        private static void OptimizeGotOreLookup(List<Chemical> distinctChemicals)
        {
    ...
    public class Chemical
    {
        ...
        public long FindCheapestReactionByOre(long quantityNeeded)
        {
            ...
            {
                var multiplier = (long) Math.Ceiling((double) quantityNeeded / InputReaction.OutputQuantity);
                var ore = InputReaction.Inputs.Sum(sm =>
                    sm.Chemical.FindCheapestReactionByOre(sm.InputQuantity * multiplier));
                var quantityFromOre = InputReaction.OutputQuantity * multiplier;
