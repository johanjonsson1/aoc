...
//jgz a -2".ToStringList();
    ...
    public class SendReceivingParser : InstructionParser
    {
        ...
        protected override int Snd(int currentIndex, string instruction)
        {
            ...
            _lastPlayed = _registers[x];
            _friendParser.Enqueue(_lastPlayed);
        ...
        protected override int Rcv(int currentIndex, string instruction)
        {
            ...
            _registers[instruction.AsChar()] = value;
    ...
    {
        protected Dictionary<char, long> _registers = new Dictionary<char, long>();
        protected readonly Dictionary<string, Func<int, string, int>> _operation =
            new Dictionary<string, Func<int, string, int>>();

        protected long _lastPlayed;

        public long RecoverValue { get; private set; }

        public InstructionParser()
        {
            _operation.Add("snd", Snd);
            _operation.Add("set", Set);
            _operation.Add("add", Add);
            _operation.Add("mul", Mul);
            _operation.Add("mod", Mod);
            _operation.Add("rcv", Rcv);
            _operation.Add("jgz", Jgz);
        }
        ...
        {
            return _operation[instruction.Substring(0, 3)]
        ...
        private int Set(int currentIndex, string instruction)
        {
            ...
            {
                _registers[x] = _registers[y.AsChar()];
            }
        ...
        protected virtual int Rcv(int currentIndex, string instruction)
        {
            ...
            if (_registers[instruction.AsChar()] != 0)
            {
                RecoverValue = _lastPlayed;
            }
        ...
        private int Jgz(int currentIndex, string instruction)
        {
            ...
            if (gtZero > 0)
            {
                ...
                {
                    //gtZero = _registers[x.AsChar()];
                    return currentIndex + (int)_registers[x.AsChar()];
                }
