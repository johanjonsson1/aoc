...
namespace AoC_2017
{
    ...
    {
        public static Point3D<Hex> CreateAdjacent(this Point3D<Hex> current, string identifier)
        {
            ...
            {
                case "n":
                    return new Point3D<Hex>(c.X, c.Y + 1, c.Z - 1, current.Item);
                case "ne":
                    return new Point3D<Hex>(c.X + 1, c.Y, c.Z - 1, current.Item);
                case "se":
                    return new Point3D<Hex>(c.X + 1, c.Y - 1, c.Z, current.Item);
                case "s":
                    return new Point3D<Hex>(c.X, c.Y - 1, c.Z + 1, current.Item);
                case "sw":
                    return new Point3D<Hex>(c.X - 1, c.Y, c.Z + 1, current.Item);
        ...
        public static int GetDistance(this Point3D<Hex> current, Point3D<Hex> target)
        {
