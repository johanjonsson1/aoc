...
public class Day8 : Day
{
    ...
    private int TotalPart2 = 0;

    public override void PartOne()
    {
        base.PartOne();

        var input = @"bfdcga cgdfe egfda cbfeda ec efc gcfbd ebcg gdfbec adcgfeb | bgdfc ec dfgae gefcd
...
ed dfgbec dfabg fbaec feacbg edafb aedc eadfcgb ebdacf dbe | deac bgeafc cebfdg fcdegb".ToStringList();

        var total = 0;
        foreach (var raw in input)
        {
            var sp = new SignalPattern(raw, true);
            total += sp.EasyMatches();
            TotalPart2 += sp.OutputValue;
        }

        Console.WriteLine(total); //239
    }
...
public class SignalPattern
{
    ...
    // 1 4 7 8
    public IEnumerable<string> EasyInputs => Inputs.Where(x => x.Length is 2 or 4 or 3 or 7);
    public IEnumerable<string> EasyOutputs => Outputs.Where(x => x.Length is 2 or 4 or 3 or 7);

    public Display CurrentDisplay { get; set; }
    public int OutputValue { get; set; }

    public SignalPattern(string raw, bool generateAll = false)
    {
        Raw = raw;
        var inputOutput = raw.Split('|');
        Inputs = inputOutput[0].Split(' ', StringSplitOptions.RemoveEmptyEntries)
            .Select(s => new string(s.Order().ToArray())).ToList();
        Outputs = inputOutput[1].Split(' ', StringSplitOptions.RemoveEmptyEntries)
            .Select(s => new string(s.Order().ToArray())).ToList();
        ...
        var one = Inputs.Single(x => x.Length == 2);
        var four = Inputs.Single(x => x.Length == 4);
        var seven = Inputs.Single(x => x.Length == 3);
        var eight = Inputs.Single(x => x.Length == 7);
        ...
        var code = string.Concat(Outputs.Select(s => CurrentDisplay.Signals.Single(x => x.Pattern == s).Digit.ToString()));
        OutputValue = int.Parse(code);
    }

    public int EasyMatches()
    {
        var output = EasyOutputs.ToList();
        var input = EasyInputs.ToList();
        var count = output.Count(o => input.Any(i => i == o));
        return count;
    }
    ...
    public class Display
    {
        ...
        public char LeftDown { get; set; }
        public char Down { get; set; }

        public List<Signal> Signals { get; set; } = new();

        public Display(Signal one, Signal four, Signal seven, Signal eight, List<string> inputs)
        {
            Signals.Add(one);
            Signals.Add(four);
            Signals.Add(seven);
            ...
            Up = seven.Pattern.Except(one.Pattern).Single();

            // 1, 2, 3, 4, 5, 6, 7, 8, 9, 0
            var findNine = new string(four.Pattern.Union(seven.Pattern).Distinct().Order().ToArray());
            var nine = inputs.Single(x => x.Length == 6 && findNine.All(x.Contains));
            var nineSignal = new Signal(9, nine);
            Signals.Add(nineSignal);
            Down = nine.Except(findNine).Single();
            LeftDown = eight.Pattern.Except(nineSignal.Pattern).Single();

            var two = inputs.Single(x => x.Length == 5 && x.Contains(LeftDown));
            var twoSignal = new Signal(2, two);
            Signals.Add(twoSignal);

            var three = inputs.Single(x => x.Length == 5 && one.Pattern.All(x.Contains));
            var threeSignal = new Signal(3, three);
            Signals.Add(threeSignal);

            var five = inputs.Single(x => x.Length == 5 && x != two && x != three);
            var fiveSignal = new Signal(5, five);
            Signals.Add(fiveSignal);

            var six = inputs.Single(x => x.Length == 6 && x != nine && five.All(x.Contains));
            var sixSignal = new Signal(6, six);
            Signals.Add(sixSignal);

            var zero = inputs.Single(x => x.Length == 6 && x != six && x != nine);
            var zeroSignal = new Signal(0, zero);
            Signals.Add(zeroSignal);
        }
    }

    public record Signal(int Digit, string Pattern);
}