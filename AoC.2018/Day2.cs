...
namespace AoC2018
{
    ...
    They'd be stored together, so the box IDs should be similar. Too bad it would take forever to search the warehouse for two similar box IDs..." 
    ...
    {
        public override string Title => "2018 - Day 2: Inventory Management System";
        ...
        public override void PartOne()
        {
            ...
            {
                var group = line.GroupBy(g => g).ToList();

                if (group.Any(a => a.Count() == 2))
                {
                    twos += 1;
                }

                if(group.Any(a => a.Count() == 3))
                {
                    threes += 1;
                }
            }

            Console.WriteLine(twos * threes); //7776
        }
