...
namespace AoC_2018
{
   ...
   --- Day 4: Repose Record ---
    ...
    public class Guard
    {
        ...
        public int MinutesAsleep => DateMinutes.Count(x => x.Action == Action.Asleep);
        ...
        public int MaxMinuteAsleep => DateMinutes.Where(x => x.Action == Action.Asleep).GroupBy(g => g.Minute)
                    .OrderByDescending(o => o.Count()).First().Key;

        public int MaxMinuteAwake => DateMinutes.Where(x => x.Action == Action.Awake).GroupBy(g => g.Minute)
                    .OrderByDescending(o => o.Count()).First().Key;
