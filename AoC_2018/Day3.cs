...
namespace AoC_2018
{
    ...
    */
    public class Day3 : Day
    {
        ...
        private readonly List<string> inputs = Inputs.Day3.ToStringList();
        private ConcurrentDictionary<SquareInch, SquareInch> claimPoints = new ConcurrentDictionary<SquareInch, SquareInch>(new SquareInchEqualityComparer());
        private ConcurrentDictionary<string, byte> allClaims = new ConcurrentDictionary<string, byte>();
        private ConcurrentDictionary<string, byte> claimsWithMatches = new ConcurrentDictionary<string, byte>();
        ...
        public override void PartOne()
        {
            ...
            Parallel.ForEach(inputs, input =>
            {
                ...
                allClaims.AddOrUpdate(id, default(byte), (k, v) =>
                {
                    return default(byte);
                });
                ...
                for (int i = 0; i < timesX; i++)
                {
                    ...
                    for (int j = 0; j < timesY; j++)
                    {
                        ...
                        claimPoints.AddOrUpdate(sqInch, sqInch, (k, v) =>
                        {
                            v.Hits += 1;

                            claimsWithMatches.AddOrUpdate(id, default(byte), (key, val) =>
                            {
                                return default(byte);
                            });

                            claimsWithMatches.AddOrUpdate(v.ClaimId, default(byte), (key, val) =>
                            {
                                return default(byte);
                            });
            ...
            var result = claimPoints.Values.Where(a => a.Hits >= 2).ToList();
        ...
        {
            base.PartTwo();
            var result = allClaims.Keys.Except(claimsWithMatches.Keys).ToList();
