...
{
    public string Title => "2017 - Day 5";
    ...
    {
        Console.WriteLine("--- Day 5: A Maze of Twisty Trampolines, All Alike ---");
...
-3";
        ...
        var stripped = Inputs.Day5.ToStringList();
        var instructions = stripped.Select(s => int.Parse(s)).ToArray();
        //var offset = new Offset(); // <-- blö ;(
        var steps = 0;
        var currentIndex = 0;

        do
        {
            var nextInstruction = instructions[currentIndex];
...
public class Offset
{
    private Dictionary<int, int> _offsetStore = new Dictionary<int, int>();

    public int Get(int offset)
    {
        if (!_offsetStore.ContainsKey(offset))
        {
            _offsetStore[offset] = offset;
        }

        var result = _offsetStore[offset];
        _offsetStore[offset] += 1;
