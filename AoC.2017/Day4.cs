...
{
    public string Title => "2017 - Day 4";
    ...
    private static void PartOne()
    {
        ...
        var rows = Inputs.Day4.ToStringList();
        var good = 0;
        var bad = 0;

        foreach (var phrase in rows)
        {
            var allWords = phrase.Trim().Split(' ').ToList();
            var distinct = allWords.Distinct().ToList();

            if (allWords.Count != distinct.Count)
            {
                bad++;
                continue;
            }
            ...
            // part two
            var ordered = allWords.Select(s => string.Concat(s.OrderBy(x => x))).ToList();
            var distinctOrdered = ordered.Distinct().ToList();

            if (ordered.Count != distinctOrdered.Count)
            {
                bad++;
                continue;
            }
