...
namespace AoC2017
{
    ...
    multiply it by a factor (generator A uses 16807; generator B uses 48271), and then keep the remainder of dividing that resulting product by 2147483647. 
    ...
    Each generator functions completely independently: they both go through values entirely on their own, only occasionally handing an acceptable value to the judge,
    and otherwise working through the same sequence of values as before until they find one.

    The judge still waits for each generator to provide it with a value before comparing them (using the same comparison method as before). 
    It keeps track of the order it receives values; the first values from each generator are compared, then the second values from each generator, then the third values, and so on.
    ...
    (Using the values from the example above, after five million pairs, the judge would eventually find a total of 309 pairs that match in their lowest 16 bits.)
    ...
    */
    ...
    {
        public override string Title => "--- Day 15: Dueling Generators ---";

        public override void PartOne()
        {
            base.PartOne();
            var generatorA = new Generator(16807, 722, 0);
            var generatorB = new Generator(48271, 354, 0);
            var score = 0;

            for (var i = 0; i < 40_000_000; i++)
            {
                if (generatorA.GetNextValue() == generatorB.GetNextValue())
                {
                    if (generatorA.LastInvalid || generatorB.LastInvalid)
                    {
                        continue;
                    }
        ...
        {
            base.PartTwo();
            var generatorA = new Generator(16807, 722, 4);
            var generatorB = new Generator(48271, 354, 8);
            var score = 0;
    ...
    public class Generator
    {
        ...
        private static int _divider = 2147483647;
        private long _value;
        ...
        public string GetNextValue()
        {
            ...
            LastInvalid = false;
            return bin.Substring(bin.Length - 16, 16);
        }

        private void CreateNextValue()
        {
            var multiplied = _value * _factor;
            _value = multiplied % _divider;
        }
    }
}