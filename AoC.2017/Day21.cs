    ...
    */
    ...
    public class Grid
    {
        ...
        public Grid(string pattern)
        {
            ...
            _grid = new char[rows.Length, rows.Length];

            for (var row = 0; row < rows.Length; row++)
            {
                for (var col = 0; col < rows.Length; col++)
                {
                    _grid[row, col] = rows[row][col];
                }
            }

            Size = _grid.GetUpperBound(0) + 1;
        }

        public override string ToString()
        {
            return Grid.ToPatternString(_grid);
        }
